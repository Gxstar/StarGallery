...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        val MIGRATION_7_8 = object : Migration(7, 8) {
            override fun migrate(db: SupportSQLiteDatabase) {
                try { db.execSQL("ALTER TABLE photos ADD COLUMN thumbnailPath TEXT") } catch (_: Exception) {}
            }
        }

        val MIGRATION_8_9 = object : Migration(8, 9) {
            override fun migrate(db: SupportSQLiteDatabase) {
                try { db.execSQL("ALTER TABLE photos ADD COLUMN lut1opacity INTEGER") } catch (_: Exception) {}
                try { db.execSQL("ALTER TABLE photos ADD COLUMN lut2opacity INTEGER") } catch (_: Exception) {}
            }
        }

        val MIGRATION_9_10 = object : Migration(9, 10) {
            override fun migrate(db: SupportSQLiteDatabase) {
                try { db.execSQL("ALTER TABLE photos ADD COLUMN locationAddress TEXT") } catch (_: Exception) {}
            }
        }

        val MIGRATION_10_11 = object : Migration(10, 11) {
            override fun migrate(db: SupportSQLiteDatabase) {
                try { db.execSQL("ALTER TABLE photos ADD COLUMN isHdr INTEGER NOT NULL DEFAULT 0") } catch (_: Exception) {}
            }
        }
